...
MainComponent::MainComponent()
{
    ...
    tempoEditor.setMouseClickGrabsKeyboardFocus(true);
    ...
    tempoEditor.setWantsKeyboardFocus(true);
    tempoEditor.setEscapeAndReturnKeysConsumed(false);
    tempoEditor.setSelectAllWhenFocused(false);
    resetTempoButton.onClick = [this]
    {
        isEditingTempoText = false;
        tempoEditor.unfocusAllComponents();
        setHostTempoBpm(getDefaultTempoBpm());
    };
...
SessionData MainComponent::buildSessionData() const
{
    ...
    session.hostSyncEnabled = standaloneTempoSupport.isHostSyncEnabled();
                                     ...
                                     juce::Array<MissingPluginEntry>& missingPlugins)
{
    ...
    standaloneTempoSupport.setHostSyncEnabled(session.hostSyncEnabled);
...
void MainComponent::resized()
{
    ...
    auto resetBounds = tempoArea.removeFromRight(32).reduced(2);
    resetBounds = resetBounds.withSizeKeepingCentre(resetBounds.getWidth(), topRow.getHeight() - 4);
...
juce::PopupMenu MainComponent::getMenuForIndex(int index, const juce::String&)
{
    ...
    switch (index)
    {
        ...
        case 2:
            ...
            menu.addSeparator();
            menu.addItem(menuHostSyncToggle, "Host Sync", true, standaloneTempoSupport.isHostSyncEnabled());
            menu.addSeparator();
...
void MainComponent::menuItemSelected(int itemId, int)
{
    ...
    switch (itemId)
    {
        ...
        case menuHostSyncToggle:
            standaloneTempoSupport.setHostSyncEnabled(!standaloneTempoSupport.isHostSyncEnabled());
            refreshTempoUiFromEngine();
            markSessionDirty();
            break;

        case menuSetFakeHostTempo:
            standaloneTempoSupport.setExternalHostTempoBpm(128.0);
            refreshTempoUiFromEngine();
            break;

        case menuClearFakeHostTempo:
            standaloneTempoSupport.clearExternalHostTempo();
            refreshTempoUiFromEngine();
            break;
...
{
    standaloneTempoSupport.setHostTempoBpm(bpm);
    updateTempoUi();
    markSessionDirty();
}

double MainComponent::getHostTempoBpm() const
{
    return standaloneTempoSupport.getHostTempoBpm();
}

double MainComponent::getDefaultTempoBpm() const
{
    return standaloneTempoSupport.getDefaultTempoBpm();
}

bool MainComponent::isHostSyncEnabled() const
{
    return standaloneTempoSupport.isHostSyncEnabled();
}

void MainComponent::registerTapTempo()
{
    standaloneTempoSupport.registerTapTempo();
    updateTempoUi();
    markSessionDirty();
}
...
{
    if (standaloneTempoSupport.isHostSyncEnabled())
    {
        if (standaloneTempoSupport.isExternalHostTempoAvailable())
            tempoEditor.setTooltip("Host Sync enabled:\nTempo controlled by host.");
        else
            tempoEditor.setTooltip("Host Sync enabled:\nWaiting for external host tempo.");

        resetTempoButton.setTooltip("Reset tempo to preset default.");
    }
    else
    {
        tempoEditor.setTooltip("Scroll to adjust.\nShift+Scroll: Fine Adjust.");
        resetTempoButton.setTooltip("Reset tempo to " + juce::String(standaloneTempoSupport.getDefaultTempoBpm(), 1) + ".");
    }
}

void MainComponent::updateTempoControlState()
{
    const bool editable = !standaloneTempoSupport.isHostSyncEnabled();
...
{
    return standaloneTempoSupport.isTempoEditorInteractive();
}
...
{
    if (!tempoEditor.hasKeyboardFocus(true))
    {
        if (standaloneTempoSupport.refreshFromEngineIfNeeded())
            refreshTempoUiFromEngine();
    }
}
...
{
    const auto displayed = juce::String(standaloneTempoSupport.getDisplayedTempoBpm(), 1);

    if (!tempoEditor.hasKeyboardFocus(true) && tempoEditor.getText() != displayed)
        tempoEditor.setText(displayed, juce::dontSendNotification);
}
