...
{
    if (state.hostSyncEnabled && state.externalHostTempoAvailable)
        hostPlayHead.setTempoBpm(state.externalHostTempoBpm);
    else
...
bool StandaloneTempoSupport::isHostSyncEnabled() const
{
    return state.hostSyncEnabled;
}

void StandaloneTempoSupport::setHostSyncEnabled(bool enabled)
{
    state.hostSyncEnabled = enabled;
    pushEffectiveTempoToEngine();
    lastDisplayedSyncedTempoBpm = -1.0;
    lastDisplayedHostTempoAvailable = state.externalHostTempoAvailable;
}
...
{
    if (state.hostSyncEnabled && state.externalHostTempoAvailable)
        return state.externalHostTempoBpm;
...
bool StandaloneTempoSupport::isExternalHostTempoAvailable() const
{
    return state.externalHostTempoAvailable;
}

void StandaloneTempoSupport::setExternalHostTempoBpm(double bpm)
{
    state.externalHostTempoBpm = juce::jlimit(20.0, 300.0, bpm);
    state.externalHostTempoAvailable = true;
    pushEffectiveTempoToEngine();
}

void StandaloneTempoSupport::clearExternalHostTempo()
{
    state.externalHostTempoAvailable = false;
    pushEffectiveTempoToEngine();
}

bool StandaloneTempoSupport::refreshFromEngineIfNeeded()
{
    if (!state.hostSyncEnabled)
        return false;

    const double currentTempo = getDisplayedTempoBpm();
    const bool hostTempoAvailable = state.externalHostTempoAvailable;
    ...
    : owner(ownerIn), beatIndicator(ownerIn), tempoEditor(*this)
{
    ...
    {
        setTempoBpm(tempoEditor.getText().getDoubleValue());
    };
...
{
    if (owner.refreshFromEngineIfNeeded())
        refreshUi();
}

void StandaloneTempoSupport::TempoStripComponent::textEditorReturnKeyPressed(juce::TextEditor& editor)
{
...
void StandaloneTempoSupport::TempoStripComponent::textEditorEscapeKeyPressed(juce::TextEditor& editor)
{
    if (&editor == &tempoEditor)
        refreshUi();
}

void StandaloneTempoSupport::TempoStripComponent::textEditorFocusLost(juce::TextEditor& editor)
{
    if (&editor == &tempoEditor)
        commitTempoFromEditor();
}
...
{
    if (owner.isHostSyncEnabled())
    {
        if (owner.isExternalHostTempoAvailable())
    ...
    else
    {
        tempoEditor.setTooltip("Scroll to adjust.\nShift+Scroll: Fine Adjust.\nDouble-click: Reset to "
                               + juce::String(owner.getDefaultTempoBpm(), 1) + ".");
    }
}
...
{
    return std::make_unique<TempoStripComponent>(*this);
}
